						...
						if(inquisidor_tipos)
							...
							switch(InquisiPref)
								...
								if("Month's Inquisitor")
									to_chat(user, "Отличник, который произвел неизгладимое впечатление на ИНКВД. Его выдающиеся достижения были признаны его начальством, и они возлагали на него большие надежды.")
